...
- [x] `T1` — pnpm workspace + root config
...
- [x] `T2` — `packages/core` package scaffold
...
- [x] `T3` — `schema.ts` + `types.ts`: VideoSpec, Scene
...
- [x] `T4` — `layers.ts`: layer union, registry, `registerLayer()`, nesting cap
...
- [x] `T5` — `diagnostics.ts`: `parseSpec`, `Diagnostic`
...
- [x] `T6` — `easing.ts`: easing catalogue
...
- [x] `T7` — `resolve.ts`: coordinate/style resolution
...
- [x] `T8` — `timeline.ts`: `compileTimeline`, `Timeline`, `activeAt`
...
- [x] `T9` — `json-schema.ts`: `generateJsonSchema()`
...
- [x] `T12` — Schema/layers/diagnostics/registry tests
...
- [x] `T13` — Resolve/easing/json-schema tests + package build verification
